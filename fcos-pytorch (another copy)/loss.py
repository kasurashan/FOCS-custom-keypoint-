...
class FCOSLoss(nn.Module):
    ...
    def prepare_target(self, points, targets):
        ...
        label, box_target, key_stacks = self.compute_target_for_location(   ############ add 
            point_all, targets, ex_size_of_interest, n_point_per_level
        )
        ...
        for i in range(len(label)):
            ...
            key_stacks[i] = torch.split(key_stacks[i], n_point_per_level, 0)  ##### add 

        label_level_first = []
        box_target_level_first = []
        key_stacks_level_first = []  ######### add
        ...
        for level in range(len(points)):
            ...
            key_stacks_level_first.append( ################### add 
                torch.cat(########
        ...
        return label_level_first, box_target_level_first, key_stacks_level_first  ####### add (keypoint)
    ...
    def compute_target_for_location(
        ...
        for i in range(len(targets)):
            ...
            key_custom = targets_per_img.key   ############### add (keypoint)
            ...
            l_key = key_custom[:, 0][None] - bboxes[:, 0][None] ############ add 
            ...
            key_stack = key_stack[range(len(locations)), locations_to_gt_id]  ##### add  
            ...
            key_stack_list.append(key_stack)   ####### add (keypoinyt)
        ...
        return labels, box_targets, key_stack_list ########### add 

    #def compute_centerness_targets(self, box_targets):
    def compute_centerness_targets(self, box_targets, key_stacks):  ########### add 

        left_right = box_targets[:, [0, 2]]
        top_bottom = box_targets[:, [1, 3]]

        left_right_key = key_stacks[:, [0, 2]] ########  add 
        ...
        l_over_r = left_right[:,0] / left_right[:,1]   # l/r ...  add (keypoint)
        t_over_b = top_bottom[:,0] / top_bottom[:,1]   # t/b ...

        l_over_r_key = left_right_key[:,0] / left_right_key[:,1]   # l/r_key ...  add (keypoint)
    ...
    def forward(self, locations, cls_pred, box_pred, center_pred, targets):
        ...
        labels, box_targets, key_stacks = self.prepare_target(locations, targets) ####### add (keypoint)
        ...
        for i in range(len(labels)):
            ...
            key_stacks_flat.append(key_stacks[i].reshape(-1, 4)) ##############  add 
        ...
        key_stacks_flat = torch.cat(key_stacks_flat, 0) ############### add 
        ...
        key_stacks_flat = key_stacks_flat[pos_id] ########## add 
